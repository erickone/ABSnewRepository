 ...
 * @author jacob.flores
 */
@Component("dependencyValidateUR")
@Scope("prototype")
public class URDependencyValidate extends CompCeilingBudget {

    @Resource(name = "DependenceDao")
    DependenceDao dependenceDao;

    public URDependencyValidate() {
        setNameValidator(ValidationNameEnum.DEPENDENCY_UR);
    }
    ...
    public List<CellCeilingBudgetDto> executeDataValidation(List<CellCeilingBudgetDto> toValidateSets) {
        ...
        List<DependenceEntity> draftProjectCatalog = dependenceDao.getViewDependenciesTypeUR();
