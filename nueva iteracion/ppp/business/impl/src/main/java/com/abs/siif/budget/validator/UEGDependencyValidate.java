 ...
 * @author jacob.flores
 */
@Component("dependencyValidateUeg")
@Scope("prototype")
public class UEGDependencyValidate extends CompCeilingBudget {

    @Resource(name = "DependenceDao")
    DependenceDao dependenceDao;

    public UEGDependencyValidate() {
        setNameValidator(ValidationNameEnum.DEPENDENCY_UEG);
    }
    ...
    public List<CellCeilingBudgetDto> executeDataValidation(List<CellCeilingBudgetDto> toValidateSets) {
        ...
        List<DependenceEntity> draftProjectCatalog = dependenceDao.getViewDepIsExecutionUnit();
